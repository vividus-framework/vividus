...
class NetworkConditionsActionsTests
{
    ...
    private WebElement changeNetworkConnection(NetworkToggle toggle, Mode mode, String toggleState)
    {
        ...
        when(driver.findElementByAccessibilityId(mode.getiOSNetworkConnectionAlias())).thenReturn(wiFiElement);
        when(driver.findElementByIosClassChain(
                String.format(XCUIELEMENT_TYPE_SWITCH, mode.getiOSNetworkConnectionAlias()))).thenReturn(switchBtn);
