...
public class ElementSteps implements ResourceLoaderAware
{
    ...
    private static final String AN_ELEMENT_WITH_THE_ATTRIBUTES = "An element with attributes%1$s";
     ...
     * @param locator to locate the element
     */
    @When("I hover mouse over element with locator `$locator`")
    public void hoverMouseOverAnElementByLocator(SearchAttributes locator)
    {
    ...
    /**
     * Checks that the <b>page</b> contains at least 1 <b>element</b> specified by <b>locator</b>
     * @param locator to locate the element
     */
    @Then("at least one element with locator `$locator` exists")
    public void doesAtLeastOneElementExist(SearchAttributes locator)
    {
        baseValidations.assertIfAtLeastOneElementExists(
                String.format("The number of elements by the locator: '%1$s'", locator), locator);
    }
     ...
     * @param childLocator locator for the child element (e.g div[@class='className'])
     */
    @Then("each element with locator `$elementLocator` has `$number` child elements with locator `$childLocator`")
    public void doesEachElementByLocatorHasChildWithLocator(SearchAttributes elementLocator, int number,
    ...
    */
     ...
     */
    @Then("each element located `$locator` has the same `$dimension`")
    ...
    */
    ...
    /**
     * Checks that an <b>element</b> located by <b>locator</b> exists in the context
     * @param locator to locate element
     * @return <b>WebElement</b> An element (field) matching the requirements,
     * <b> null</b> - if there are no desired elements
     */
    @Then("element located `$locator` exists")
    public WebElement findElementBy(SearchAttributes locator)
    {
        return baseValidations.assertIfElementExists(String.format(AN_ELEMENT_WITH_THE_ATTRIBUTES, locator), locator);
    }

    /**
     * Checks that an <b>element</b> located by <b>locator</b> does not exist in the context
     * @param locator to locate element
     */
    @Then("element located `$locator` does not exist")
    public void elementDoesNotExist(SearchAttributes locator)
    {
        baseValidations.assertIfElementDoesNotExist(String.format(AN_ELEMENT_WITH_THE_ATTRIBUTES, locator), locator);
    }
    ...
    {
        WebElement webElement = findElementBy(locator);
    ...
    public void clickOnAllElements(SearchAttributes locator)
    {
        baseValidations.assertIfElementsExist("The elements to click", locator)
                .forEach(e -> clickActions.click(e));
    }
