...
public final class CeilingJsCoordsProvider extends CoordsProvider
{
    ...
    private CeilingJsCoordsProvider(String scrollTopJs, IJavascriptActions javascriptActions)
    {
        ...
        getCoordinatesJs = String.format("var rect = arguments[0].getBoundingClientRect();"
                ...
                + "var margin ="
                + " {"
                + "  left: getAttrbuteValue(style, \"margin-left\"),"
                + "  top: getAttrbuteValue(style, \"margin-top\"),"
                + "  right: getAttrbuteValue(style, \"margin-right\"),"
                + "  bottom: getAttrbuteValue(style, \"margin-bottom\")"
                + " };"
                + "function getAttrbuteValue(style, attributeName){return Math.max(0, parseInt(style[attributeName]))}"
                + "var scrollTop = %s;"
