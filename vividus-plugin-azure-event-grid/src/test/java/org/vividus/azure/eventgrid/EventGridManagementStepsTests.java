...
class EventGridManagementStepsTests
{
    ...
    Stream<DynamicTest> shouldRetrieveKeyVaultProperties()
    {
        var urlPath = String.format(
                "subscriptions/%s/resourceGroups/%s/providers/Microsoft.EventGrid/systemTopics",
                SUBSCRIPTION_ID_PROPERTY_VALUE, RESOURCE_GROUP_NAME);
        return DynamicTest.stream(createStreamOfTests(),
                test -> test.accept(steps -> steps.listSystemTopics(RESOURCE_GROUP_NAME, SCOPES, VAR_NAME),
                urlPath)
        );
    }

    @SuppressWarnings("PMD.CloseResource")
    private String testHttpRequestExecution(Consumer<EventGridManagementSteps> test, int statusCode,
                                            String expectedUrlPath)
    {
        ...
        try (MockedStatic<HttpPipelineProvider> httpPipelineProviderMock = mockStatic(HttpPipelineProvider.class))
        {
            ...
            var steps = new EventGridManagementSteps(azureProfile, tokenCredential, softAssert, variableContext);
            test.accept(steps);
            var httpRequest = httpRequestCaptor.getValue();
            assertEquals(HttpMethod.GET, httpRequest.getHttpMethod());
            assertEquals("https://management.azure.com/" + expectedUrlPath + "?api-version=2021-12-01",
