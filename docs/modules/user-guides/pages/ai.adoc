...
NOTE: The examples below cover GitHub Copilot in VS Code and Claude Desktop, but any https://modelcontextprotocol.io/clients[MCP-compatible client] (such as Cursor, Windsurf, or Continue) can be configured in the same way using the VIVIDUS MCP server command shown here.
...
[%collapsible]
====
Claude Desktop is a desktop client for Anthropic's Claude models.
...
NOTE: Copilot skills are picked up by the https://docs.github.com/en/copilot/using-github-copilot/using-copilot-coding-agent/about-copilot-coding-agent[GitHub Copilot coding agent] (the cloud-based agent that is assigned to GitHub Issues). They are not used by the Copilot in VS Code agent mode chat.
...
.  Restart the MCP server (in VS Code click `⟳ Restart` next to the server name; in Claude Desktop restart the application).
