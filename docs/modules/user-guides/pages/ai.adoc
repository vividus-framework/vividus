...
.   Place the ready-made prompt files into the created folder. The sample prompt for generation of VIVIDUS stories testing web-applications is available in the https://github.com/vividus-framework/vividus-sample-tests/blob/main/.github/prompts/generate-vividus-story-web.prompt.md[sample project], the content should be copied to the local project and then can be modified depending on the needs and specifics of SUT.
+
[IMPORTANT]
The prompt file must have the `.prompt.md` extension (e.g., `tgenerate-vividus-story-web.prompt.md`). This extension ensures proper
...
.Combine the prompt set previously with a detailed test case:
```
...
.Or combine the prompt set previously with general Web testing scenarios (scenarios list generated by AI):
```
