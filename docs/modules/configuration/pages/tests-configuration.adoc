...
TIP: Table row filtering can be paired up with xref:ROOT:commons:table-transformers.adoc#_resolving_variables_eagerly[RESOLVING_VARIABLES_EAGERLY transformer]
...
[source,properties]
----
http.auth.myapp.origin=https://securedomain.com
http.auth.myapp.username=whoisthis
http.auth.myapp.password=3db8a57ba925a522c8bf
http.auth.myapp.preemptive-auth-enabled=false
----
====

|`http.auth.<group id>.origin`
|character sequence
|`*`
a|The URL identifying the origin to which configured basic authentication credentials will apply e.g. `https://securedomain.com`.
All the URL components besides `scheme` and `domain` contained in the URL are ignored.

To apply basic authentication credentials to all domains use `*` (asterisk) symbol as an origin value. Please note that more
specific origin values take presendence over `*` value.

IMPORTANT: Presence of this property requires `http.auth.<group id>.username` and `http.auth.<group id>.password` properties to be set as well

|`http.auth.<group id>.username`
|character sequence
|`<empty>`
a|The username

IMPORTANT: Presence of this property requires `http.auth.<group id>.origin` and `http.auth.<group id>.password` properties to be set as well

|`http.auth.<group id>.password`
|character sequence
|`<empty>`
a|The password

IMPORTANT: Presence of this property requires `http.auth.<group id>.username` and `http.auth.<group id>.origin` properties to be set as well

|`http.auth.<group id>.preemptive-auth-enabled`
|`boolean`
|`false`
