...
|`false`
|If `true` and any failure occurs during the batch execution, the subsequent batches will not be executed.
...
|`false`
|If set to `true` prints failures in a tree-like structure including: story, scenario, step and error message
