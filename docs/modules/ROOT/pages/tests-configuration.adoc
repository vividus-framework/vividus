...
* Scenarios' grouping in Allure report.
* Scenarios' delimitation for test executing by selected tag.
* Setting desired execution attributes on scenario-level.

=== Levels
Meta tags may be used at both story and scenario level. Meta tags specified at Story level are propagated to every scenario, if the same meta tag is used at Scenario level, then Scenario level value is applied.
----
Meta:
     @group Login

Scenario: Check redirection to Online Stores
Meta:
  @testTier 1
  @layout desktop
...
----
selects all stories/scenarios with tag @group Login and without @skip.
...
|[subs=+quotes]`@proxy`
|Some test scenarios require Proxy server mechanism switched-on, for this purpose test should be marked with this tag to let Vividus test framework understand that proxy mechanism should be switched-on first as a precondition for test execution.
...
|@testTier 1 / @testTier 2 / ... / @testTier 4

|@group
|Any desired value
|This tag is used to group marked stories and/or scenarios under specified label in Allure report.
|@group Login
