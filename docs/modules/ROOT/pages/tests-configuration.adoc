...
VIVIDUS configuration includes the following parts: profiles, environments and suites.
...
The suites are loaded one by one starting from the last one in the sequence, in other words, the values of the properties defined in the first suite will take precedence over the values of the same properties defined in all subsequent suites. If any of the suites in the sequence is not found (i.e. no `*.properties` file are present at the given path), the error will be thrown and the execution will be stopped. However if suites property is empty (`configuration.suites=`) and no suite is configured in the root (i.e no files can be found by `/properties/suite/*.properties` pattern), no error will be thrown and the execution will be continued.
