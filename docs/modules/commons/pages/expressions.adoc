...
== Files
:file-path-parameter: pass:quotes[*`filePath`* - the path to the file]

=== `loadFile`

Loads the file by its path and replaces the expression with the content of the file.
...
[source,gherkin]
----
When I initialize the scenario variable `my-data` with value `#{loadFile(data/body.txt)}`
----

=== `loadBinaryFile`

Loads the file by its path as bytes
...
.Load data from the file as bytes
[source,gherkin]
----
When I initialize the scenario variable `my-data` with value `#{loadBinaryFile(data/body.txt)}`
----
