...
NOTE: The expression fails if the file does not exist or the path points to a directory.

.Load data from a file on the file system
[source,gherkin]
----
Given I initialize scenario variable `my-data` with value `#{loadFile(/home/user/data/body.txt)}`
----

=== `loadBinaryFile`

Loads binary file by its path.
...
.Load binary data from a file on the file system
[source,gherkin]
----
Given I initialize scenario variable `my-data` with value `#{loadBinaryFile(/home/user/data/image.png)}`
----
