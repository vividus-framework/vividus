...
`RESOLVING_EXPRESSIONS_EAGERLY` processes expressions which are used as values inside table.

[IMPORTANT]
====
By using expression for random data generation:
* without `RESOLVING_EXPRESSIONS_EAGERLY` - newly generated value will always be substituted;
* with `RESOLVING_EXPRESSIONS_EAGERLY` - value will be generated once and it will be substituted the same in all places.
====
...
`RESOLVING_SELF_REFERENCES_EAGERLY` allows to reuse table data inside itself.
