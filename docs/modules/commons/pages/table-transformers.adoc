...
`RESOLVING_EXPRESSIONS_EAGERLY` processes expressions which are used inside `ExamplesTable` eagerly, i.e. at the table loading stage.
...
`RESOLVING_SELF_REFERENCES_EAGERLY` enables eagerly resolution of references to one column from another column within single `ExamplesTable` ("eagerly" means at the table loading stage). Circular references will result in error.
