...
* `$filePath` - The full file path, including its name and extension (e.g., C:\Users\Public\Documents\example.txt).

.Verify file existence validation
[source,gherkin]
----
Given I initialize scenario variable `file-path` with value `${java.io.tmpdir}/example.txt`
