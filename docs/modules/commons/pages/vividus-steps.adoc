...
=== Execute steps and assert duration

Executes the steps located in the table, measures their execution duration and asserts it against the expected duration
using the comparison rule.
...
* `$comparisonRule` - xref:parameters:comparison-rule.adoc[The comparison rule].
* `$duration` - The expected duration in {durations-format-link} format.
* `$stepsToExecute` - The xref:ROOT:glossary.adoc#_examplestable[ExamplesTable] with a single column containing the steps to execute.

.Execute steps and verify they complete in less than 5 seconds
[source,gherkin]
----
...
Execute steps if the variable with specified name is not set.
...
=== Execute steps if a variable is set

Execute steps if the variable with specified name is set.
...
* `$name` - The variable name to check.
* `$stepsToExecute` - The xref:ROOT:glossary.adoc#_examplestable[ExamplesTable] with a single column `step` containing the steps to execute if variable `$name` is set.

.Execute steps if variable `user` is already set
[source,gherkin]
----
When variable `user` is set I do:
| step                                          |
| When I perform some action for logged in user |
----
...
Think of an object pool as a shared basket of items that your tests can borrow from. You fill the basket with a
...
grab the same account and interfere with each other. If the basket runs low, you can add more items to it later.
...
Puts more items into a basket that already exists. Use this to top up a basket that is running low or empty, without
creating a new one. Adding items to a basket that has not been initialized stops the test with an error. Every row of
the table becomes one more item, and the column headers become the details of that item. Newly added items are shuffled
and placed after the items that are already in the basket.

[source,gherkin]
----
Given I add objects to object pool `$poolName`:$data
----
* `$poolName` - The name of the basket to add items to. The basket must already exist.
* `$data` - The xref:ROOT:glossary.adoc#_examplestable[table] of items to add. Each row is one item.

.Add two more user accounts to the "credentials" basket
[source,gherkin]
----
Given I add objects to object pool `credentials`:
|login |password|
|user-4|pass-4  |
|user-5|pass-5  |
----
...
Saves how many items are still available in the basket. Items that a test has already taken are not counted until they
are returned. Use the saved number to walk through every remaining item. Asking for the size of a basket that has not
been initialized stops the test with an error. An empty basket saves `0`.

The saved number is a snapshot. If another test adds items to the same basket at the same time, the number can be
inaccurate.
...
----
* `$poolName` - The name of the basket to count items in.
* `$scopes` - xref:commons:variables.adoc#_scopes[How long the stored number should stay available] (for example, for
the current scenario or the whole story).
* `$variableName` - The name you give to the number so you can use it later.

.Take every remaining account from the "credentials" basket
[source,gherkin]
----
When I save size of object pool `credentials` to scenario variable `credentialsCount`
When I execute steps while counter is less than `${credentialsCount}` with increment `1` starting from `0`:
|step                                                                                    |
|When I take object from pool `credentials` and save it to scenario variable `credential`|
|Then `${credential.login}` matches `user-[1-5]`                                         |
----

The counter starts at `0` and stops before the saved size, so each remaining item is taken once. Using
`less than or equal to` would try to take one more item than the basket has and stop the test.
