...
It's allowed to use `global`, `next batches`, `scenario` and `story` variables within templates by referring them using the variable reference notation. Note that the parameters passed to the step take precedence over the variables.
