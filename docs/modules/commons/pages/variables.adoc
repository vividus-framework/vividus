...
|`batch`
a|Such variables are declared only via suite properties with bdd.variables.batch-<batch-number>. prefix and available throughout the execution of the batch in the suite. An attempt to create the batch scoped variables during test execution leads to an error.
