...
* Values from map variables can be accessed by key. For instance, `+${listVar.key1}+` returns the value associated with 'key1'.

==== Working with Tables

List and map variables can be combined into more complex structures. A common case is tables, which are represented as a list of maps.

Each Map object represents a row, with the keys corresponding to column headers and the values being the cell data.
These Map objects are added to a List, creating a collection of rows representing the entire table.
...
* `+${tableVar[1]}+` returns a Map of the second table row: [|A:A2|, |B:B2|].
