...
.Retrieve file via SFTP protocol
[source,gherkin]
----
...
* `$parameters` - The xref:ROOT:glossary.adoc#_examplestable[ExamplesTable] with SSH port forwarding and connection parameters.
+
NOTE: The parameters marked with *bold* are mandatory.
...
|[subs=+quotes]`*local-port*`
|0..65535
|
|The local TCP port to bind on the client machine. Incoming connections to this port are sent through the SSH tunnel

|[subs=+quotes]`*remote-port*`
|0..65535
|
|The TCP port on the `remote-host` that receives the forwarded traffic

|[subs=+quotes]`*remote-host*`
|hostname or IP
|
|The target host (reachable from the SSH server) to which the forwarded traffic should be delivered. This can be an internal host not directly accessible from the client
