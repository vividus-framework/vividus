...
Find blobs with names filtered by the specified rules in the container.
...
* `$filter` - Table with filter rules to apply on blob names.
+
...
|`resultsLimit`
|Maximum number of blob names to return.
|`10`

|`blobNameFilterRule`
|xref:parameters:string-comparison-rule.adoc[The blob name comparison rule].
Should be specified along with ```blobNameFilterValue```.
|`MATCHES`

|`blobNameFilterValue`
|The full or partial blob name to be matched.
Should be specified along with ```blobNameFilterRule```.
|`.*`
|===
