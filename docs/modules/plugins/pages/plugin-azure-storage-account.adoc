...
[source,gherkin]
----
When I upload blob with name `$blobName` and `$data` to container `$containerName` of storage account `$storageAccountKey`
----
...
[source,gherkin]
----
When I upload blob with name `raw/test.jon` and `{"key":"value"}` to container `vividus` of storage account `testaccount`
----
