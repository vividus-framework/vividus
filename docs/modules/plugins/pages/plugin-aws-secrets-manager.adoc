...
[source,properties]
----
AWS_SECRETS_MANAGER(<profile>, <secret>/<secret_name>:<default_value>)
----
+
Where:
+
...
+
* `default_value` - *[optional]* The value to use when the secret exists in AWS Secrets Manager, but the requested `secret_name` is not present in the secret JSON. May be empty (`secret/secret_name:`). An info message is logged when the default value is applied. If the secret itself is missing, an error is thrown.
+
[source,properties]
----
...
db.connection.test.host=AWS_SECRETS_MANAGER(my_profile, my_secret/dev/host:localhost)
----
