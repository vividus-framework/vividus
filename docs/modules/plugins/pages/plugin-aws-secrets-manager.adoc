...
|`true`
|Controls the activation of the processor that resolves property values from Secrets Manager. If set to `false`, values from AWS Secrets Manager will not be retrieved. Can be used when access to the Secret Manager is restricted, e.g. for local debugging, to override only the required properties instead of all processed ones.
