...
|[subs=+quotes]`*secrets-manager.azure-key-vault.name*`
|
...
|`true`
|Controls the activation of the processor that resolves property values from Secrets Manager. If set to `false`, values from Azure Key Vault will not be retrieved. Can be used when access to the Secret Manager is restricted, e.g. for local debugging, to override only the required properties instead of all processed ones.

|===

===== How to get Azure Key Vault name
. Find the required Key Vault in the list.
+
image::azure_key_vaults_list.png[Azure Key Vaults List]

. Put particular Azure Key Vault name as value for property `secrets-manager.azure-key-vault.name`
+
[source,properties]
----
secrets-manager.azure-key-vault.name=testautokveus01
----

===== How to refer Azure Key Vault secrets
. Find the required secret in Azure Key Vault.
+
image::azure_key_vault.png[Secrets in Azure Key Vault]
