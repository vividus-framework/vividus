...
|The timeout in {iso-date-format-link} format.
|PT5M
...
NOTE: The saved stdout/stderr are limited to ~2 billions of characters. But keep in mind depending on run configuration and environment it's possible to hit memory limits before reaching described output limits.
