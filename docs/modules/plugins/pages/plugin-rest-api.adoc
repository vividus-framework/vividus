...
4+^.^|_Configuration of HTTP authentication_
...
4+^.^|_Configuration of SSL handshake_
...
4+^.^|_General HTTP configuration_
...
4+^.^|_Configuration of Two-Way SSL authentication_
...
4+^.^|_Configuration of DNS resolver_

|`http.local-dns-storage.`
|`key-value mapping`
|`<empty>`
|The property family to override the OS DNS lookup by specifying hostname to IP address mappings e.g. `http.local-dns-storage.coursera.org=52.84.197.20`

4+^.^|_Configuration of retry handler_
