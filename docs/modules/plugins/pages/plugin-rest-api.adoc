...
`step`
|`global`
a|The property that allows to configure level of storing cookies during execution requests:

* `global` - maintains cookies across the entire test suite and across all threads. Cookies set in one test are available and shared in all subsequent tests until cleared or the suite completes.
* `story` - cookies are saved only for the duration of a single story and cleared at the beginning of a new story.
* `scenario` - reset cookies after each scenario.
* `step` - reset cookies after each step.
