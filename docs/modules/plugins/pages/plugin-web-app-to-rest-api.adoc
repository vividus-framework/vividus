...
Set current browser cookies to the HTTP client for future executing API requests
...
Set current cookies from the previous API calls to the browser.
Make sure that expected cookies from API calls matches with web browser opened page domain.
...
Set current cookies from the previous API calls to the browser, but does not apply the changes in cookies instantly.
