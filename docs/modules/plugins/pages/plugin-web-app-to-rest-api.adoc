...
. `$locatorType` - The `CSS selector` or `XPath`.
. `$locator` - The locator to locate resources for validation.
. `$pages` - The pages to validate resources on.

.Validate resources located by XPath
[source,gherkin]
----
Then all resources located by xpath `//a` are valid on:
|pages                        |
|https://vividus.org/         |
...
[source,gherkin]
----
Then all resources located by css selector `a,img` in ${source-code} are valid
----
