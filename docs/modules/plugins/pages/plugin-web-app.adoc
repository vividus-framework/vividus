...
Step changes the current browser window size to the specified one.

[IMPORTANT]
Specified browser window size should be smaller than the current screen resolution.
...
* `$targetSize` - Any browser window size in pixels, e.g. 800x600,
where first measure is window WIDTH, last measure is its HEIGHT.
