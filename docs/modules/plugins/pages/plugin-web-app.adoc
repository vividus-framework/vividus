...
Web-application plugin contains significant amount of built-in profiles. They can be found after plugin installation by the following path in Eclipse IDE:
[source, DIR]
----
...
General place for web profiles. Each of the directory contains browser related properties. This is a place for adjustments for your browser. Add there any properties you need: screen resolution, path to webdriver or to a custom browser.
...
selenium.grid.platform-version=10
----
To run tests in Opera browser locally it is required to set the following property:
[source,properties]
----
web.driver.OPERA.binary-path=<path to opera>
----
* <path to opera> - path to the Opera browser (for example, "C:/Programs/Opera/opera.exe")
...
* `$duration` - The timout duration to wait for a window in {iso-date-format-link} format.
