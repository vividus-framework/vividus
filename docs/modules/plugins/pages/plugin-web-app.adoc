...
* `$duration` - The timeout duration to wait for a window in {durations-format-link} format.
...
==== Save the number of open browser tabs

This step allows you to save the count of currently open tabs in the browser to a variable.
...
.Save the number of open tabs into a variable named 'count'
[source,gherkin]
----
When I save number of open tabs to STORY variable `count`
----
...
Values are stored in the variable as a list of maps. For more information, see xref:commons:variables.adoc#_working_with_tables[working with table variables]
...
[NOTE]
Before using this step, ensure the test context is set to the required table. See the example below.
