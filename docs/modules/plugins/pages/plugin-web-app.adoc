...
Parses https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URLs[Data URLs] and decodes their data if https://developer.mozilla.org/en-US/docs/Glossary/Base64[Base64] encoding is set. The result of the expression depends on the Data URL's https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types[MIME type]. The result will be a binary data in case of the binary MIME type and it will be a string for the text MIME type.
...
|`#{decodeDataUrl(data:text/plain;base64,SGVsbG8sIFdvcmxkIQ==)}`  |`Hello, World!`
|`#{decodeDataUrl(decodeDataUrl(data:image/png;base64,iVBORw....)}|Image binary data.
|===
