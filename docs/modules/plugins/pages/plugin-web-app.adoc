...
Gets a list of elements by locator and check that it is sorted by text with using sortingOrder. The list should have more than 1 element.
If some elements don't contain text that test will be broken.
...
. `$locator` - <<_locator>>
. `$sortingOrder` - xref:parameters:sorting-order-rule.adoc[Sorting order]
...
Given I am on a page with the URL 'https://vividus-test-site.herokuapp.com/sortedListOfElement.html'
