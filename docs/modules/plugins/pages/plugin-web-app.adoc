...
:selenium: true
include::partial$common-web-app-mobile-emulation-steps.adoc[conditional="selenium"]

include::partial$common-web-app-network-conditions-steps.adoc[]
...
When running web tests on real mobile devices or emulators/simulators, it may be necessary to interact with components
from the native mobile context (e.g., permission pop-ups). The following steps can be used to achieve this.
...
.Allow to use device location on Android
[source,gherkin]
----
...
Switches context to a web view with name that matches the specified comparison rule.
...
.Allow to use device location on iOS
[source,gherkin]
----
Given I am on page with URL `https://the-internet.herokuapp.com/geolocation`
When I click on element located by `buttonName(Where am I?)`
...
==== Execute steps in mobile native context
...
original context. The key advantage of this step is that it is not needed to know the name of the original context.
...
=== Typing large text into the field

Sometimes, large amounts of text may be entered into the field very slowly, only partially,
or not at all. This happens because the system types the text character by character.
With large texts, this becomes inefficient, may cause performance issues, timeouts, or truncation.

The following workarounds can be used:

.Add large text to the field in chunks
[source,gherkin]
----
When I execute steps while counter is less than or equal to 10 with increment 1 starting from 1:
|step|
|When I add `#{generate(regexify '[a-z]{3000}')}` to field located by `id(description)`|
----

.Use clipboard instead of typing large text
[source,gherkin]
----
When I execute javascript `navigator.clipboard.writeText("#{generate(regexify '[a-z]{30000}')}");`
