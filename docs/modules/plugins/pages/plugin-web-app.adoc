...
Step should be used before start of the browser session in order to take effect.

List of supported command line arguments for popular browsers:
. https://peter.sh/experiments/chromium-command-line-switches/[Chrome]
. https://www-archive.mozilla.org/docs/command-line-args[Firefox]
. https://docs.microsoft.com/en-us/previous-versions/windows/internet-explorer/ie-developer/general-info/hh826025(v=vs.85)[Iexplore]
...
* `$argsString` - Sequence of command line arguments, separated by ` `.
