...
* `random()` - returns a random element from the input array.
* `random(n)` - returns an array of `n` random elements from the input array.

.Examples of `random()` JSON path expression
|===
|JSON |JSON Path |Result
...
NOTE: The `random()` function returns a different element on each execution, so the actual result will vary.
...
|columns
a|The semicolon-separated list of mappings between column names and {json-paths} used to get column values. The semicolons can optionally be followed by newlines for better readability.

.Columns parameter format (single line)
[source]
----
columns=column_name_1=json_path_1;column_name_2=json_path_2;column_name_N=json_path_N
----

.Columns parameter format (multi-line)
[source]
----
columns=
column_name_1=json_path_1;
column_name_2=json_path_2;
column_name_N=json_path_N
----
