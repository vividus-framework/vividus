...
|`lighthouse.performance-validation-strategy`
|`NONE`, `HIGHEST`, `LOWEST`
|`NONE`
a|Lighthouse performance scores are changed due to inherent https://developers.google.com/web/tools/lighthouse/variability[variability] in web and network technologies, even if there hasn't been a code change, to alleviate these effects this property allows to run several Lighthouse measurements and perform metrics validation on a measurement selected by user.

Available options:

* `HIGHEST` - validation is performed against Lighthouse measurement with the highest `performance score`, can be used only if `performance` category is specified in `lighthouse.categories` property.
* `LOWEST` - validation is performed against Lighthouse measurement with the lowest `performance score`, can be used only if `performance` category is specified in `lighthouse.categories` property.
* `NONE` - Lighthouse measurement is performed only once.

|`lighthouse.performance.measurements-number`
|positive number
|3
|Specifies the number of Lighouse measurements performed before validation, the property takes its effect only if `lighthouse.performance-validation-strategy` is set to either `HIGHEST` or `LOWEST`.
