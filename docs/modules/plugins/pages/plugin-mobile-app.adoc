...
|`mobile-environment.ios.use-workaround-to-hide-keyboard`
 a|`true` +
 `false`
 ...
 a|
Controls whether an iOS-specific workaround to hide the on-screen keyboard is applied. When enabled (default), VIVIDUS
attempts to dismiss the keyboard by tapping the last button on the iOS keyboard when it is shown.
For `XCUIElementTypeTextView` on real devices the workaround is not applied; tap outside the text view to close the
keyboard. Disable the workaround (`false`) to rely solely on the underlying Appium behavior.

[CAUTION]
====
This workaround may slow down test execution.
====
...
:hybrid-apps-info: (see https://appium.github.io/appium-xcuitest-driver/latest/guides/hybrid/[Automating hybrid iOS apps] and https://github.com/appium/appium-uiautomator2-driver?tab=readme-ov-file#hybrid-mode[Automating hybrid Android apps] for more information)

=== Switch to Web view by name

Switches context to a web view with name that matches the rule {hybrid-apps-info}. Also, Appium capabilities for https://appium.github.io/appium-xcuitest-driver/latest/reference/capabilities/#web-context[iOS] and https://github.com/appium/appium-uiautomator2-driver#web-context[Android] are available for the fine-tuning configuration.
...
Switches context to a mobile native context {hybrid-apps-info}.
...
is performed to close the keyboard. The behavior is controlled by
`mobile-environment.ios.use-workaround-to-hide-keyboard` property;
set it to `false` to rely solely on the underlying Appium behavior.
