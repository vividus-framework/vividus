...
Navigates back in the mobile application
...
[source,gherkin]
----
Scenario: Go back from login page
Then number of elements found by `accessibilityId(login)` is equal to `1`
When I navigate back
Then number of elements found by `accessibilityId(login)` is equal to `0`
----
