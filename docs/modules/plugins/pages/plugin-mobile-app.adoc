...
|`mobile-application.full-screen`
a|`true`

`false`
|`false`
|Set to `true` if mobile application is running in full-screen mode and system status bar is not displayed.
...
Runs mobile application in the backround for specific period of time and either activates the app after a certain amount of time, or leave the app deactivated (when PT-1S is used). See https://appium.io/docs/en/commands/device/app/background-app/[Appium docs] for more details.
