...
Calculates the difference between two dates with given formats and returns result as {iso-date-format-link} format.
...
* *`$unixEpochTime`* - the Unix epoch time to be converted to the {iso-date-format-link} format.
