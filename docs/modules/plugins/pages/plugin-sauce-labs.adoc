...
|
|This property comes into effect only if proxy is used, otherwise its value is ignored. The property specifies the list of glob patterns for hosts that are connected-to directly, others are connected through the proxy. By default the list inicludes `*.miso.saucelabs.com`, `*.api.testobject.com`, `*.saucelabs.com`, `saucelabs.com` host patterns to avoid sending of status information by SauceConenct to SauceLabs REST API throughout the lifetime of a tunnel via proxy used by tests.
