...
[source,gherkin]
----
When I $actionType baseline with name `$name` using storage `$storage` and ignoring:$ignoredElements
----
...
[source,gherkin]
----
When I $actionType baseline with name `$name` using storage `$storage` and ignoring:$ignoredElements and screenshot configuration:$screenshotConfiguration
----
