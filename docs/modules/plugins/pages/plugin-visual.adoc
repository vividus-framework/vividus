...
|~size in pixels~
|~Native footer to cut.~ *DEPRECATED* use `mobile.screenshot.strategy.<YOUR_STRATEGY_NAME>.cut-bottom`.
...
mobile.screenshot.strategy.custom.shooting-strategy=SIMPLE
...
=== Run visual test using image out of file

Establishes baseline or compares against existing one using image out of file.
...
[source,gherkin]
----
When I compare_against baseline with name `context` from image out of file `/Users/Foo/Downloads/context.png`
----
...
=== Run simple visual test with custom baseline storage using image out of file

Establishes baseline or compares against existing one using xref:developer-guides:plugins.adoc#_baseline_storages[baseline storage].

[source,gherkin]
----
When I $actionType baseline with name `$name` from image out of file `$filePath` using storage `$storage`
----
...
[source,gherkin]
----
When I compare_against baseline with name `context` from image out of file `/Users/Foo/Downloads/context.png` using storage `filesystem`
----
...
=== Run visual test with ignoring option using image out of file

Establishes baseline or compares against existing one using the ignoring option.

[source,gherkin]
----
When I $actionType baseline with name `$name` from image out of file `$filePath` ignoring:$ignoredElements
----
...
[source,gherkin]
----
When I compare_against baseline with name `context-element-with-acceptable-diff-percentage` from image out of file `/Users/Foo/Downloads/context.png` ignoring:
...
=== Run visual test using image out of file and ignoring option and baseline storage

Establishes baseline or compares against existing one using the ignoring option and xref:developer-guides:plugins.adoc#_baseline_storages[baseline storage].

[source,gherkin]
----
When I $actionType baseline with name `$name` from image out of file `$filePath` using storage `$storage` and ignoring:$ignoredElements
----
...
[source,gherkin]
----
When I compare_against baseline with name `context-element-with-acceptable-diff-percentage` from image out of file `/Users/Foo/Downloads/context.png` using storage `filesystem` and ignoring:
|ACCEPTABLE_DIFF_PERCENTAGE|
|20                        |
----
