...
See the official https://docs.aws.amazon.com/sdk-for-java/v2/developer-guide/credentials-chain.html["Working with AWS Credentials"] guide to get more details.

=== Region Selection

The plugin attempts to find AWS region by using https://docs.aws.amazon.com/sdk-for-java/v2/developer-guide/region-selection.html[the default region provider chain]. The provider chain looks for a region using the provided below options one by one starting from the top. If region is found at some point, the search stops and further options are not evaluated.
...
See the official https://docs.aws.amazon.com/sdk-for-java/v2/developer-guide/region-selection.html["AWS Region Selection"] guide to get more details.
