...
:log-entries: List of the comma-separated messages levels. The supported levels are: ERRORS, WARNINGS, INFOS.
...
This step gets the identifier of the currently active tab and then switches focus to the next tab after it. For example, if tabs #1, #2, #3 are open and tab #2 is active, this step will switch focus to tab #3. If the active tab is the last one, the step switches to the first tab.
