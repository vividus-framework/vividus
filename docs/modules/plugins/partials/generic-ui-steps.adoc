...
[WARNING]
In case when xref:plugins:plugin-web-app.adoc#_visibility_types[locator's visibility] and checked state are equal (For an example ':i' and 'NOT_VISIBLE') exception will be thrown. In such cases please use step: <<_verify_elements_number>>.
...
* `$duration` - The period of time the element should have existed in {iso-date-format-link} format.
