...
class SshStderrDynamicVariableTests
{
    @Mock private SshTestContext sshTestContext;
    @InjectMocks private SshStderrDynamicVariable dynamicVariable;

    @Test
    void shouldReturnErrorStream()
    {
        var sshOutput = new SshOutput();
        sshOutput.setErrorStream("error");
        when(sshTestContext.getSshOutput()).thenReturn(sshOutput);
        assertEquals(DynamicVariableCalculationResult.withValue(sshOutput.getErrorStream()),
