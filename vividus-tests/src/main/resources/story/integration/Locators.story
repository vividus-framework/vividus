...
When I save `id` attribute value of element located by `tagName(button)->filter.index(2)` to SCENARIO variable `elementId`
Then `${elementId}` is equal to `button-show`
When I save `id` attribute value of element located by `tagName(button)->filter.attribute(id=button-show)` to SCENARIO variable `elementId`
Then `${elementId}` is equal to `button-show`
When I save `id` attribute value of element located by `tagName(div):a->filter.state(not visible)` to SCENARIO variable `elementId`
