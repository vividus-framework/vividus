...
SET SCHEMA test;
CREATE TABLE if not exists test.launch_rockets (name VARCHAR(50) NOT NULL, payload_cost VARCHAR(100), country VARCHAR(100) NOT NULL);
...
When I execute SQL query `UPDATE test.launch_rockets SET payload_cost=NULL WHERE country='USSR';` against `testh2db-modifiable`
...
Then `${test-data}` is equal to `[{name=Proton, payload_cost=null, country=USSR}]`
Then `${test-data[0].payload_cost}` is equal to `#{null}`
