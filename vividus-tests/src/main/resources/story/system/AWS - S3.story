...
When I upload data `Hello from Vividus!` with key `${object-key}_text` and content type `text/plain` to S3 bucket `vividus-bucket`
When I upload data `#{loadResource(/data/csv.csv)}` with key `${object-key}_csv` and content type `text/csv` to S3 bucket `vividus-bucket`

Scenario: Fetch the created object
When I fetch object with key `${object-key}_text` from S3 bucket `vividus-bucket` and save result to scenario variable `content`
Then `${content}` is equal to `Hello from Vividus!`

Scenario: Set the created object ACL
Meta:
    @requirementId 1210
When I set ACL `aws-exec-read` for object with key `${object-key}_text` from S3 bucket `vividus-bucket`
...
Then `${keys[0]}` is contains `${object-key}`
Then `${keys[1]}` is contains `${object-key}`
When I collect objects keys filtered by:
|filterType                      |filterValue   |
|key suffix                      |.csv          |
|object modified not earlier than|${start-time} |
in S3 bucket `vividus-bucket` and save result to scenario variable `keys2`
Then `${keys2[0]}` is equal to `${object-key}_csv`

Scenario: Delete the created object
When I delete object with key `${object-key}_text` from S3 bucket `vividus-bucket`
When I delete object with key `${object-key}_csv` from S3 bucket `vividus-bucket`
