...
Examples:
|action         |firstP             |batchName           |hash      |
|COMPARE_AGAINST|xpath((.//p)[1])   |Vividus System Tests|25eb674c89|


Scenario: Validation of step: 'When I $actionType baseline `$testName` in batch `$batchName` with Applitools' for full page
When I <action> baseline `full-page-<hash>` in batch `<batchName>` with Applitools
...
When I <action> baseline `context-<hash>` in batch `<batchName>` with Applitools

Scenario: Validation of CHECK_INEQUALITY_AGAINST action
When I change context to element located by `<firstP>`
When I CHECK_INEQUALITY_AGAINST baseline `full-page-<hash>` in batch `<batchName>` with Applitools
...
|full-page-element-cut-<hash>|<batchName>|<action>|<firstP>        |
...
|full-page-area-cut-<hash>|<batchName>|<action>|<firstP>     |
...
When I run visual test with Applitools using:
|baselineName              |batchName  |action  |elementsToIgnore|
|context-element-cut-<hash>|<batchName>|<action>|<firstP>        |
...
When I run visual test with Applitools using:
|baselineName                           |batchName  |action  |elementsToIgnore   |
|not-viewport-context-element-cut-<hash>|<batchName>|<action>|By.cssSelector(img)|
...
|full-page-with-scroll-element-area-cut-<hash>|<batchName>|<action>|By.xpath(//p[position() mod 2 = 1 and position() > 10])|By.xpath(//p[position() mod 2 = 1 and position() < 10])|
...
|scrollable-element-context-<hash>|<batchName>|<action>|
...
When I run visual test with Applitools using:
|baselineName                                   |batchName  |action  |elementsToIgnore                                      |areasToIgnore                                         |
|scrollable-element-fullpage-with-ignores-<hash>|<batchName>|<action>|By.xpath(//a[position() mod 2 = 1 and position() > 7])|By.xpath(//a[position() mod 2 = 1 and position() < 7])|
 and screenshot config:
|scrollableElement|webHeaderToCut|webFooterToCut|scrollTimeout|
|By.id(scrollable)|10            |0             |PT1S         |
